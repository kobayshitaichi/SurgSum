...
class SumLitModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        loss, mse, mae = self._shared_step(batch)
        ...
        logger.info(f'train_mae {batch_idx}: {mae}')
    ...
    def validation_step(self, batch, batch_idx):
        ...
        logger.info(f'val_mse {batch_idx}: {mae}')
    ...
    def test_step(self, batch, batch_idx):
        with torch.no_grad():
            ...
            self.weight = weight.cpu().detach().numpy()
            self.gts = gts.cpu().detach().numpy()
